...
- 종류
  - SimpleRetryPolicy
    - 지정한 예외 타입에 대해 최대 횟수(maxAttempts)만큼 재시도하는 가장 기본적인 정책
  - TimeoutRetryPolicy
    - 지정된 기간(timeout) 동안만 재시도를 허용하는 정책
    - 네트워크 지연, 외부 API 응답 대기 같은 시나리오에서 유용
  - AlwaysRetryPolicy
    - 어떤 예외든 제한 없이 재시도하는 정책
    - 실무에서는 거의 사용되지 않는다.
  - NeverRetryPolicy
    - 어떤 예외든 재시도하지 않는 정책
    - 재시도 자체가 비즈니스에 부정적일 때 사용할 수 있다.
      - 중복 처리나 일관성 문제로 재시도가 되면 안되는 경우
...
### 성능 최적화 (Reader 개선)
- Reader의 성능을 개선하여 배치 애플리케이션의 성능을 높일 수 있다.
- 데이터를 읽는 방법은 다양하게 구성할 수 있다.
  - Limit-Offset 방식
    - OFFSET + LIMIT을 사용하여 데이터를 건너뛴 후 타겟 데이터만 가져오는 방식
    - 구현이 매우 간단하다.
    - 데이터를 순차적으로 읽은 후 필요한 데이터를 제외하고 버리는 방식이기 때문에 페이지가 뒤로 갈수록 느려진다.
  - No-Offset 방식
    - 마지막 키를 관리하고 있다가 PK 기반으로 조회를 한다. 
    - 인덱스 기반 탐색을 하기 때문에 성능이 균등하다.
  - Cursor 방식
    - 클라이언트 사이드와 서버 사이드가 존재한다.
    - 클라이언트 사이드
...
### 성능 최적화 (Processor 개선)
- I/O 병목 최소화를 위해 Processor 로직을 Writer로 이전하여 일괄 처리한다.
- 이전 후 Writer 내에서 외부 API 호출 등은 RX Java 등으로 병렬화하여 처리 속도를 향상시킨다.

### 성능 최적화 (Writer 개선)
- 데이터를 그룹화하여 WHERE IN 절로 한번에 업데이트 (DB I/O 감소)
- 불필요한 데이터베이스 접근을 제거하여 처리 속도 향상

### 성능 최적화 (Partition Step)
- 병럴 Partition Step 처리
- MasterStep, Slave Step 수행 구조

### 파티셔닝 (Partitioning)
- 파티셔닝이란?
  - 거대한 하나의 일감(단일 Step)을 여러 개의 작은 일감(파티션)으로 잘게 쪼갠 뒤, 여러 명의 작업자(스레드)에게 나누어주고 동시에 병렬로 처리하게 하는 스프링 배치의 가장 강력한 성능 최적화 기능이다.
  - 시스템이 가진 멀티코어 CPU 자원을 100% 활용하여 대용량 데이터 처리 시간을 획기적으로 단축시킬 수 있다.
  - 수백만 건의 데이터를 처리할 때, 메모리 문제를 피하면서도 가장 빠르게 작업을 끝낼 수 있는 방법 중 하나이다.
- 아키텍처
  - 파티셔닝 아키텍처는 작업을 분할하고 지시하는 '매니저 스텝'과, 지시를 받아 실제 데이터를 처리하는 '워커 스텝'이라는 두 가지 명확한 역할로 구성
- 매니저 스텝 (Manager Step)
  - 매니저 스텝은 파티셔닝의 컨트롤 타워이다.
  - 직접 데이터를 처리하지 않고, 전체 작업을 분할하고 워커 스텝들을 관리 및 실행하는 책임을 가진다.
  - 주요 책임
    - 파티션 생성 (Partitioner)
      - Partitioner는 전체 데이터 범위를 어떻게 나눌지 결정하는 로직을 담은 컴포넌트입니다.
      - JobParameters를 참고하여, 각 워커가 처리할 데이터의 범위(예: 날짜, ID 범위)를 ExecutionContext에 담아 여러 개의 파티션을 생성한다.
    - 워커 스텝 실행
      - 생성된 파티션의 수만큼 워커 스텝을 실행하도록 스케줄링한다.
    - 병렬 처리 (TaskExecutor)
      - TaskExecutor (스레드 풀)를 사용하여 워커 스텝들을 병렬로 동시에 실행시킨다.
      - gridSize 옵션으로 동시에 실행할 워커의 최대 개수를 제어할 수 있다.
- 워커 스텝 (Worker Step)
  - 워커 스텝은 매니저로부터 특정 데이터 처리 임무를 할당받은 실제 작업 단위이다.
  - 각 워커는 자신에게 주어진 범위의 데이터만 독립적으로 처리한다.
  - 주요 책임
    - 데이터 수신
      - 각 워커 스텝은 시작될 때, 매니저가 파티셔너를 통해 생성한 StepExecutionContext를 전달받는다.
      - 코드에서는 @StepScope와 @Value 어노테이션을 사용하여 이 컨텍스트에 담긴 파라미터를 주입받는다.
    - 독립적 실행
      - 각 워커는 독립적인 Reader, Processor, Writer 인스턴스를 가진다.
      - 이를 통해 다른 워커의 작업에 전혀 영향을 주지 않고 자신만의 데이터 처리를 수행한다.
    - 트랜잭션 관리
